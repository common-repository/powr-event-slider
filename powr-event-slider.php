<?php
    /**
     * @package POWr Event Slider
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Event Slider
    Plugin URI: https://www.powr.io/plugins/event-slider
    Description: Showcase upcoming events with images and videos in this responsive slider!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Event Slider widget/////////////////////////////////
    class Powr_Event_Slider extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_event_slider',
                             __( 'POWr Event Slider' ),
                             array( 'description' => __( 'Event Slider by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-event-slider' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Event Slider.
        </p>
        <p>
    ...
    function register_powr_event_slider() {
      register_widget( 'Powr_Event_Slider' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_event_slider' );
    //Create short codes for adding plugins anywhere:
    function powr_event_slider_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-event-slider' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-event-slider' label='$label'></div>";
      }else{
      	"<div class='powr-event-slider'></div>";
      }
    }
    add_shortcode( 'powr-event-slider', 'powr_event_slider_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_event_slider_menu' );
    function powr_event_slider_menu() {
      add_menu_page( 'POWr Event Slider', 'POWr Event Slider', 'manage_options', 'powr-event-slider-settings', 'powr_event_slider_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_event_slider_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/event-slider';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/event-slider';
      }
    ...
    register_activation_hook(__FILE__, 'powr_event_slider_plugin_activate');
      add_action('admin_init', 'powr_event_slider_plugin_redirect');

      function powr_event_slider_plugin_activate() {
      add_option('powr_event_slider_plugin_do_activation_redirect', true);
      }
      ...
      function powr_event_slider_plugin_redirect() {
      if (get_option('powr_event_slider_plugin_do_activation_redirect', false)) {
          delete_option('powr_event_slider_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-event-slider-settings&' );
          }
       }
      }
